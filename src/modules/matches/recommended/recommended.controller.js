import { getRecommendedMatchesService } from './recommended.service.js';
import { MATCHES_ERRORS } from '../matches.errors.js';
import logger from '../../../logger/logger.js';

export const getRecommendedMatchesController = async (req, res) => {
  try {
    const { sortBy, search, page, limit, ...filters } = req.query;

    const userId = req.userId;

    const response = await getRecommendedMatchesService({
      userId,
      filters,
      sortBy,
      search,
    ...
    return res.status(200).json({
      success: true,
      message: 'Recommended matches fetched successfully',
      data: response,
    });
  } catch (error) {
    ...
    logger.error({ err: error }, 'Failed to fetch recommended matches');
